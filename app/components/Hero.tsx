...
import { ChevronLeft, ChevronRight, Plus, Trash2, X, ImagePlus } from 'lucide-react';

const Hero: React.FC = () => {
    const { banners, isLoading, addBanner, updateBanner, deleteBanner } = useHeroBannersApi();
    const { isEditMode } = useEditMode();

    const [plugins] = useState(() => [Autoplay({ delay: 4500, stopOnInteraction: false, stopOnMouseEnter: true })]);
        ...
        return (
            <section className="w-full h-[280px] md:h-[420px] lg:h-[500px] bg-gradient-to-br from-[#0f2b4a] to-[#1a4a7f] flex items-center justify-center relative">
                <div className="text-center text-white/60">
                    ...
                    {isEditMode && <p className="text-sm mt-1 opacity-70">Click "Add Banner" below to get started</p>}
                </div>
                {isEditMode && (
                    <button
                        ...
                        style={{ background: 'linear-gradient(135deg, #153D6A, #1a4a7f)', boxShadow: '0 4px 16px rgba(21,61,106,0.5)' }}
                    >
                ...
                <div className="fixed inset-0 z-[9990] flex items-center justify-center bg-black/50 backdrop-blur-sm">
                    <div className="bg-white rounded-2xl shadow-2xl p-6 max-w-md w-full mx-4 max-h-[90vh] overflow-y-auto">
                        <div className="flex items-center justify-between mb-5">
                            <h2 className="text-lg font-bold text-gray-800">Add Banner Image</h2>
                            <button onClick={() => { setShowAddModal(false); setNewImage(null); setNewImagePreview(null); setNewCaption(''); }} className="text-gray-400 hover:text-gray-600">
                                <X size={20} />
                            </button>
                        </div>
                        <form onSubmit={handleAddSubmit} className="space-y-4">
                            <div>
                                ...
                                {newImagePreview ? (
                                    ...
                                    <label className="flex flex-col items-center justify-center h-40 border-2 border-dashed border-gray-300 rounded-xl cursor-pointer bg-gray-50 hover:bg-gray-100 transition-colors">
                                        ...
                                        <span className="text-sm text-gray-500 font-medium">Click to select image</span>
                                        <span className="text-xs text-gray-400 mt-1">Recommended: 1400×500 px</span>
                                        <input
                                            type="file"
                                            accept="image/*"
                                            className="hidden"
                            ...
                            <div className="flex gap-3 pt-2">
                                <button type="button" onClick={() => { setShowAddModal(false); setNewImage(null); setNewImagePreview(null); setNewCaption(''); }} className="flex-1 py-2.5 px-4 rounded-xl bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-medium transition-colors">Cancel</button>
                                <button type="submit" disabled={saving || !newImage} className="flex-1 py-2.5 px-4 rounded-xl bg-[#153D6A] hover:bg-[#1a4a7f] text-white text-sm font-medium transition-colors disabled:opacity-50">{saving ? 'Uploading...' : 'Add Banner'}</button>
                            </div>
                        </form>
                    </div>
                </div>
